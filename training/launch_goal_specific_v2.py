"""Launch ``training.train_goal_specific_v2`` workers across all GPUs.

64 variants × 7 goals = 448 jobs sharded round-robin onto 16 GPUs (28 each).
Each variant trains in ~2-3 min (1 epoch, 200-rollout subsample) so total
wall-clock should be under an hour.

Usage:
    python -m training.launch_goal_specific_v2          # 16 GPUs, 64 variants
    python -m training.launch_goal_specific_v2 --dry-run
    python -m training.launch_goal_specific_v2 --n-variants 32
"""
...
DEFAULT_LOGS = (
    "/mnt/pccfs2/backed_up/jeffolmo/goal-detector/logs/goal_specific_v2"
)
...
def main() -> None:
    ...
    print(f"[launch] {len(pairs)} (goal, variant) jobs across {len(gpus)} GPUs "
          f"({args.n_variants} variants per goal × {len(PIPELINE_GOAL_INDICES)} goals)")
    ...
    for gpu, pp in gpu_pairs.items():
        ...
        cmd = [
            sys.executable, "-u", "-m", "training.train_goal_specific_v2",
            "--pairs", pairs_arg,
