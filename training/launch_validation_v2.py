"""Launch ``training.validate_goal_specific`` on the 448 v2 adapters.

Same shape as ``training.launch_validation`` but enumerates 64 variants per
goal pointing to ``goal_specific_v2`` checkpoints. Outputs go to
``results/goal_specific_validation_v2``.
"""
...
DEFAULT_OUT_DIR = (
    "/mnt/pccfs2/backed_up/jeffolmo/goal-detector/results/goal_specific_validation_v2"
)
DEFAULT_LOGS = (
    "/mnt/pccfs2/backed_up/jeffolmo/goal-detector/logs/validation_v2"
)
...
def main() -> None:
    ...
    print(f"[launch] {len(pairs)} validation jobs across {len(gpus)} GPUs "
    ...
    for gpu, pp in gpu_pairs.items():
        ...
        cmd = [
            sys.executable, "-u", "-m", "training.validate_goal_specific",
            "--pairs", pairs_arg,
