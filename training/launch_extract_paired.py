"""Launch ``training.extract_paired_activations`` workers across all GPUs.

Per-model wall clock ~ (n_relevant_compounds × n_envs × ~0.8s/rollout). With
the default 30 envs and 4-6 relevant compounds, that's ~100-150s per model.
448 models / 16 GPUs = 28 models per worker → ~50-70 min total.
"""
...
from training.launch_rollouts import PIPELINE_GOAL_INDICES
...
DEFAULT_LOGS = (
    "/mnt/pccfs2/backed_up/jeffolmo/goal-detector/logs/paired_activations_v2"
)


def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--gpu-list", default=",".join(str(i) for i in range(16)))
    p.add_argument("--models-dir", default=DEFAULT_MODELS_DIR)
    p.add_argument("--out-dir", default=DEFAULT_OUT_DIR)
    p.add_argument("--log-dir", default=DEFAULT_LOGS)
    p.add_argument("--n-variants", type=int, default=64)
    p.add_argument("--n-envs", type=int, default=30)
    p.add_argument("--keepers-json", default=None,
                   help="optional path to JSON list of (attr,val,variant) "
                        "triples filtered by validation; otherwise enumerate "
                        "all variants")
    ...
    print(f"[launch] {len(pairs)} paired-extraction jobs across {len(gpus)} GPUs "
          f"({args.n_envs} envs per compound)")
    ...
    for gpu, pp in gpu_pairs.items():
        ...
        cmd = [
            sys.executable, "-u", "-m", "training.extract_paired_activations",
            "--pairs", pairs_arg,
